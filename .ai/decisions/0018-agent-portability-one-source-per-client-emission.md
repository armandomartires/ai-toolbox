...
### `worktree-only` on Claude Code — measured 2026-09-26 (`TASK-0107`)

`TASK-0056` ran this and was **confounded** by permission denials, which are
indistinguishable from confinement seen from outside. Re-run with denial
reporting as an explicit requirement. **Nothing was denied** — the probe
reports `DENIALS: NONE` — so the measurement is clean this time.

**The question asked:** does a `worktree`-isolated subagent's commit reach the
real tree? **Measured answer: no.** A subagent spawned with Claude Code's
`isolation: "worktree"` committed `0c12686` on branch
`worktree-agent-<id>`; verified afterwards **from the main checkout**, not
from the subagent's report: `master` stayed at `5d51e2d`, `git merge-base
--is-ancestor 0c12686 master` was false, and the probe file was absent from
the main working tree.

**Two observations the question did not ask for, and they are the load-bearing
ones:**

1. **The object database and ref namespace are shared.** `git cat-file -t
   0c12686` in the main checkout returns `commit`, and the branch listed in
   the main checkout's `git branch`. Isolation separates working trees and
   branch tips; it does not partition the repository.
2. **The worktree is created *inside* the repository**, at
   `.claude/worktrees/agent-<id>/`. `docs/operations/runbook.md` forbids that
   placement for this repo's own `worktree.sh` — "Never put one inside the
   repo" — because `tests/validate.sh` and `scripts/sync-registry.sh` glob
   component directories and `git status` would see it. Checked, both hold
   here: the globs are one level deep and did not recurse, `validate.sh: OK`
   and the registry regenerated byte-identical. `git status` stayed clean
   **only because `TASK-0106` had just gitignored `.claude/`** — that is luck,
   not design, and it matters: `Driver.step1_preflight_repo()` refuses a dirty
   tree, so on a checkout without that ignore an unattended run using
   worktree-isolated subagents would refuse to start.

**This does not settle the emission, and the term table's line 134 stands
unchanged.** `worktree-only` is an **access-denial** term — OpenCode's
`external_directory: deny` stops a role *reading or writing* outside its
worktree. What was measured is **effect-isolation**: where a commit lands and
what the default working directory is. The probe's worktree sat inside the
main repository, so nothing stopped it reaching the main checkout by absolute
path; that was not tested and must not be assumed. Isolation confines the
*accidental* — a relative-path write, a commit on the wrong branch — and says
nothing about the *deliberate*, which is what the term exists to deny.

**So clause 8.2 still applies**: a role declaring `worktree-only` still has no
per-agent Claude Code target, and emission for that target must still fail
