...
| **F6** | `scripts/emit-agents.py` emits all nine roles for their declared clients with exit 0, and **refuses** when an OpenCode-only role is widened to `claude-code`. | **UNTESTED** — `TASK-0059`/`install.sh` settle it. Note the refusal path *was* exercised incidentally by `TASK-0071`, which observed `EMISSION REFUSED` for a `test-allowlist` role widened to `claude-code`. | Running `install.sh` for both clients. | The emitter's refusal path is broken, which is worse than the roles being wrong — ADR-0018 clause 8.2's whole point. Prove the refusal before trusting any pass. |
