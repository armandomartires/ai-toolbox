...
**Four items are open, and they are not open in the same sense** — one
fewer than the five recorded here before, because `B-026` closed 2026-09-23
(`TASK-0060`). The distinction this paragraph used to lose is still the
point: it once counted three while B-024…B-026 sat `ready` in the table
above it.
...
- **Unscheduled and unclaimed: none.** `B-027` closed by `TASK-0074` and
  ...
  by reading the repo.
- **Ready but already scoped into S9's briefs: B-024** (`TASK-0059`).
  Not available work; work with a task file waiting on a sprint that has not
  opened. **B-026 was the other, and closed 2026-09-23 by `TASK-0060`** — the
  first S9-scoped row to close, and it closed against a *worse* defect than it
  described, because `TASK-0075` had taken the OpenCode-only roles from three
  of six to four of six earlier the same day.
