...
| B-025 | No vocabulary term for "may call only this MCP server" — `B-021`'s sibling on the MCP side | medium | medium | B-024 (the server that makes it concrete) | low | **waiting** | **Re-statused 2026-09-25 from `ready` to `waiting`, on the human's choice to leave it open:** its own condition below — a *second* role that wants the term — is still unmet (only `gate-runner` does), so `ready` was telling a planner it was workable when by its own terms it is not. Raised 2026-09-23 by `PLAN-0006`. The ten-term capability vocabulary can say `bash-allowlist`, but there is no way for a role to declare that it may reach **one** MCP server and no other tool. This is why `gate-runner` is authored as OpenCode-only rather than given an MCP-shaped Claude Code equivalent: `scripts/emit-agents.py` writes only `Agent(...)` entries into `tools:`, with a comment recording why naming a concrete tool is unsafe — it would silently remove every tool *not* named. So a Claude Code `gate-runner` would ship with `Bash` intact, which is a weaker boundary than the one its description would imply. **Needs a decision, not a widened list**, exactly as `B-021` says of its own case: a term defined in `docs/development/authoring-guide.md` first, enforced in `validate.sh` second, mapped in `emit-agents.py` third (`ADR-0008`), with per-term client coverage stated (`ADR-0018` clause 8.1 — a term is not admitted merely because it can be written). Ready when someone has a second role that wants it; one instance is a case, two is a vocabulary |
...
| B-029 | Roles are denied reading the `unattended-ops` skill their bodies cite | medium | high | none | low | **done** | **Closed 2026-09-25 by `TASK-0101`**, on the human's choice to stop pointing roles at it: seven run-role bodies keep the citation as provenance but no longer instruct a read, and every OpenCode prompt says the skill's files are outside the worktree and not to be opened. Return shapes were already in every prompt in both bindings. **It surfaced `B-035`**: the adjudicator's decision standard is not. Raised 2026-09-24 by `REVIEW-0012` from `TASK-0092` findings 7 and 13. OpenCode's `external_directory` rule denied every read under `~/.claude/skills/unattended-ops/` and `~/.config/opencode/skills/unattended-ops/` — **57 of 92 denied calls in one run**, each a wasted model round — while role bodies say to read `references/return-schemas.md` there. The driver's prompts carry every return shape, so nothing broke. Two routes, the human's choice: allow the skill path per role, or stop pointing role bodies at it |
| B-030 | A role with `read` can open the gate map | high | high | none | low | **done** | **Closed 2026-09-25 by `TASK-0098`**, by the second route: the OpenCode driver refuses a gate map inside the repository, so every role's `worktree-only` (`external_directory: deny`) keeps it unreadable — and `run-gate.sh` no longer copies each gate's argv into `<run root>/gates/<handle>/spec.json`, **a second leak inside the tree that moving the map alone would have missed**. The Claude Code binding is not covered: its `worktree-only` is `isolation: worktree`, which redirects rather than refuses. Raised 2026-09-24 by `REVIEW-0012` from `TASK-0092` finding 10. No prompt carried a command, but the pilot's refuter opened `gates.json` with its `read` tool, so rule 2 is structural in prompts only. Candidate fixes: a per-role `read` deny for the map's path, or keeping the map outside the worktree the roles work in |
| B-031 | Nothing checks what the closer writes in a task file's log | medium | high | none | low | **done** | **Closed 2026-09-25 by `TASK-0099`**, on the human's choice of a fixed placeholder: the OpenCode driver hands the closer two `log_lines` (`Commit: pending — recorded at landing (run <id>)`, `Push: not taken — the run pushes nothing`) and, after the commit, halts on any added `Commit:`/`Push:` entry that is not one of them, or on either missing. The hash is landing's — `docs/operations/runbook.md` says to replace the placeholders in a follow-up commit. **Limit, stated:** free prose outside those two labelled entries is still unchecked. Raised 2026-09-24 by `REVIEW-0012` from `TASK-0092` finding 17. The closer wrote *"Commit: recorded by the closer role in a follow-up commit"* into both pilot task files and made no follow-up commit; the refuter had already run. Corrected by hand in `53dba10` |
| B-032 | The Claude Code binding does not check a closed commit's file list | medium | medium | none | low | **done** | **Closed 2026-09-25 by `TASK-0103`**: `verifyHead()` now asks the preflight witness for `git show --name-only --format= HEAD`, and `step10Close()` halts on any file outside the declared paths, or on no list. Reported by a second agent, since the script cannot run git — a witness, as the binding already says of the other facts. Raised 2026-09-24 by `REVIEW-0012`. `TASK-0097` added the check to the OpenCode driver; the Claude Code template's `verifyHead()` reads HEAD, parent, porcelain and message only |
| B-033 | Role timeouts are unsized, and a run has no clean stop | medium | medium | none | low | **done** | **Closed 2026-09-25 by `TASK-0100`.** The OpenCode binding takes an optional `role_timeouts:` mapping (unknown roles and unreadable durations refused), with the pilot's measurements as sizing guidance rather than pre-filled numbers; SIGTERM/SIGINT/SIGHUP to the driver kills the current role call's process group, retries nothing and writes a *STOPPED* handover without another model call, leaving uncommitted work in the tree; a role process killed from outside halts rather than being retried. **Limit, stated:** a gate already started runs on to its own watchdog. Raised 2026-09-24 by `REVIEW-0012` from `TASK-0092` findings 9 and 11. `role_timeout: 10m` was too short for a thorough refuter (27–40 model rounds); killing a role call makes the driver retry it, and a killed driver writes no handover |
| B-034 | Permission matching depends on quoting; roles keep reaching for denied utilities | low | medium | none | low | **done** | **Closed 2026-09-25 by `TASK-0102`.** `scripts/emit-agents.py` appends a generated *Shell commands you may run* section to every OpenCode role with a shell allowlist — its patterns verbatim, the common utilities it denies, and *write path arguments in double quotes* — so the list keeps one owner, the frontmatter. **The upstream report is drafted in `TASK-0102` for the human to file** (the human's choice); not filed. Raised 2026-09-24 by `REVIEW-0012` from `TASK-0092` findings 12 and 16. `git add -- .ai/x` denied, `git add -- ".ai/x"` allowed; roles repeatedly tried `cat`, `ls`, `grep`, `sed`, `echo`, one `find /`. Worth a role-body note listing the allowed commands, and an upstream report |
| B-035 | The adjudicator's decision standard does not reach it inside a run | medium | high | none | low | **ready** | Raised 2026-09-25 by `TASK-0101` while closing `B-029`. `skills/unattended-ops/references/verdicts.md` (the five definitions, their misuse, the park-versus-halt-run test, the decision order) and `references/evidence.md` are the adjudicator's standard, and **neither is in its prompt** in the OpenCode binding — which carries the enum and the return shape only — and the skill is outside the role's worktree. The pilot's adjudicator could not read them either and still decided both tasks correctly, so nothing was lost by `TASK-0101`; the body now says so, and that an unsettled case parks. **Needs a route, the human's:** the driver embeds the two references' text in the adjudicator's prompt (one owner kept — the file is read, not copied), or the standard is accepted as body-plus-enum and `verdicts.md` stops being described as what the role applies |

**Two items are open**: `B-035`, `ready` (raised 2026-09-25 by `TASK-0101`), and `B-025`, `waiting` on a second role. `B-029`…`B-034` closed 2026-09-25 by `TASK-0098`…`TASK-0103`. Six — `B-029`…`B-034` — were raised 2026-09-24 by
`REVIEW-0012` from the S10 pilot's findings. The seventh, `B-025`, is open in
the "waiting on evidence" sense rather than the "waiting on a planner" one.
Before those six, the count was one — two fewer than the three recorded here
earlier: `B-026` closed 2026-09-23 (`TASK-0060`) and `B-024` the
