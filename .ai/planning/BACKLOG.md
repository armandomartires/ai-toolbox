...
| B-026 | `ADR-0018` clause 8.5 is unsatisfied — the registry shows no client coverage | medium | medium | none | low | **ready** | Raised 2026-09-23 by `PLAN-0006`, but the defect dates to S7. Clause 8.5 is normative and reads *"The registry must show a role's client coverage, so an OpenCode-only role is visible as such rather than appearing universal."* It has never been implemented. `docs/registry.md`'s Agents section is `\| Name \| Description \| Path \|`, and **three of the six existing roles are OpenCode-only** — `git-ops`, `qa-test`, `review` — while the registry presents all six identically. **S9 makes it actively misleading rather than merely incomplete**: `TASK-0064` adds five more OpenCode-only roles, taking the total to eight of fifteen, so a reader would be told fifteen universal roles exist when seven cannot be emitted for Claude Code. That is `B-021`'s defect class at the index level, where a reader is least likely to check. The mechanism already exists — the MCP section carries a fourth column (Shape), so `emit_section` supports one. Note `sync-registry.sh` records why agents got **no** Shape column (`mode` is self-declared rather than derived, and `ADR-0005`'s Clarification prefers derived); `clients` is self-declared too, but unlike `mode` it is the field the **emitter acts on** and `validate.sh` already constrains it to a closed set. That distinction must be written into the script's header, or a later reader will read the earlier decision as simply reversed. Scoped as `TASK-0060`, and it must land **before** `TASK-0064` |

**Three items are open — B-018, B-021** (raised 2026-09-16 by REVIEW-0008)
**and B-023** (raised 2026-09-23 by TASK-0051). B-023 is the first item
raised from a **seam between two briefs** — work each explicitly assigned to
the other, visible only once both had run.
