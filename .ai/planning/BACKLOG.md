...
| B-033 | Role timeouts are unsized, and a run has no clean stop | medium | medium | none | low | **ready** | Raised 2026-09-24 by `REVIEW-0012` from `TASK-0092` findings 9 and 11. `role_timeout: 10m` was too short for a thorough refuter (27–40 model rounds); killing a role call makes the driver retry it, and a killed driver writes no handover |
...
**Five items are open** (`B-030`, `B-031` closed 2026-09-25 by `TASK-0098`, `TASK-0099`). Six — `B-029`…`B-034` — were raised 2026-09-24 by
