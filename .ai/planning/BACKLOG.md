...
| B-034 | Permission matching depends on quoting; roles keep reaching for denied utilities | low | medium | none | low | **ready** | Raised 2026-09-24 by `REVIEW-0012` from `TASK-0092` findings 12 and 16. `git add -- .ai/x` denied, `git add -- ".ai/x"` allowed; roles repeatedly tried `cat`, `ls`, `grep`, `sed`, `echo`, one `find /`. Worth a role-body note listing the allowed commands, and an upstream report |
...
**Four items are open** (`B-029`…`B-031`, `B-033` closed 2026-09-25 by `TASK-0098`…`TASK-0101`; `B-035` raised by `TASK-0101`). Six — `B-029`…`B-034` — were raised 2026-09-24 by
