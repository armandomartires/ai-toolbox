...
> **CLOSED 2026-09-22 by `REVIEW-0010` (verdict: approve), recorded by
> `TASK-0054`.** The human **ratified ADR-0014, ADR-0015 and ADR-0016** on
> the same date, in answer to the review's ratification packet — the one act
> the checkpoint named as blocking closure, and the only thing outstanding
> once all seven tasks and all five exit criteria were met.
>
> **All four of this sprint's backlog items (B-010…B-013) are closed** — the
> first fully-cleared slice in this repo. Its highest-value deliverable, the
> `gather_subset` guard, was **re-run at closure rather than re-read**:
> `tests/gather-subset-guard.sh` → **10/10 PASS**, on a toolchain that had
> itself drifted underneath it (`ansible-core` 2.20.8 → 2.21.4).
>
> **The checkpoint's headline finding was not predicted by anyone**: the
> target repo is no longer re-checkable at the commit this sprint read it at.
> The constraint held — that tree is clean — but exit criterion 5's
> *verification method* cannot be reproduced today. Two things absorbed it:
> the guard's harness is hermetic, and `TASK-0032` recorded the estate facts
> in this repo rather than as pointers, for an unrelated reason. See
> `REVIEW-0010` findings 1 and 2.
>
> **Four follow-ups outlive this sprint** and are the review's, not this
> file's: the gate's still-undecided budget (its second checkpoint
> unactioned), the nine recorded copies of a moved `ansible-core` version,
> whether anything should re-check external claims at all, and
> `skills/ansible-ops/` remaining unexercised against a live estate.
>
> **S8 is now current.** Everything below is the sprint as it ran.
...
> ~~ratification of the three ADRs, and a checkpoint~~ — **both now done:
> the checkpoint is `REVIEW-0010` and all three ADRs were ratified
> 2026-09-22.** Struck rather than deleted, because it dates what was
> outstanding.
>
...
| ADR-0014 | TASK-0027 | **accepted 2026-09-22** | Accept and narrow the MCP surface; ADR-0010 stays closed. **Body written 2026-09-16; ratification owed** |
| ADR-0015 | TASK-0027 | **accepted 2026-09-22** (**retitled**) | ~~Portable core + per-project templates~~ → **derive per change, persist nothing**; check+snapshot, not staging. **Clause 1 reversed on evidence**; filename deliberately unchanged |
| ADR-0016 | TASK-0028 | **accepted 2026-09-22** (**retitled**) | Hooks as a component category — ~~expected "no"~~ **declined, but because the clients disagree on the tool's *name*, not because interception fails; it works in both** |
