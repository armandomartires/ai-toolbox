...
| 4 — nothing is invented | Evidence-line cross-check in `Driver.gate_report()`; out-of-enum verdict → `park`; the closer's `Commit:`/`Push:` entries must be the driver's fixed `log_lines`, checked after the commit (`TASK-0099`) | **Partly** — a figure copied into a task file by the closer, and free prose outside those two labelled entries, are prompt-level |
